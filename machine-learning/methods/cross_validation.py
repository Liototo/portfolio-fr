...
def splitting_fn(data, labels, indices, fold_size, fold):
    ...
    left_ind = fold * fold_size
    right_ind = (fold + 1) * fold_size
    val_indices = indices[left_ind:right_ind]
...
def cross_validation(method_obj=None, search_arg_name=None, search_arg_vals=[], data=None, labels=None, k_fold=4):
    ...
    for arg in search_arg_vals:
        ...
        for fold in range(k_fold):        
            ...
            res_val = method_obj.predict(val_data)
            acc_list2.append(metric(res_val, val_label))
            
        acc_list2 = np.array(acc_list2)
        acc_list1.append(acc_list2.mean())
        
    best_ind = find_param_ops(acc_list1)
    best_hyperparam = search_arg_vals[best_ind]
    best_acc = acc_list1[best_ind]
