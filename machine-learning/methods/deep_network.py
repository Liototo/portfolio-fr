...
class Trainer(object):
    ...
    def eval(self, dataloader):
        ...
        self.model.eval()
        results_class = torch.empty(0)
        with torch.no_grad():
            for it, batch in enumerate(dataloader):
                x, y, z = batch
                predicted = self.model(x).argmax(axis=1)
                results_class = torch.cat((results_class, predicted))

        results_class = torch.tensor(results_class).int()
